...
import com.pubmatic.spark.util.collection.unsafe.sort.{PrefixComparators, RecordPointerAndKeyPrefix, UnsafeSortDataFormat}
...
class ExternalSorterSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("force to spill for external sorter") {
    ...
    sc = new SparkContext("local", "test", conf)
    val N = 200000
    val p = new com.pubmatic.spark.HashPartitioner(2)
    val p2 = new com.pubmatic.spark.HashPartitioner(3)
