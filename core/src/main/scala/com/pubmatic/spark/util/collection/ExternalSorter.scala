...
package com.pubmatic.spark.util.collection
...
import com.pubmatic.spark.shuffle.ShufflePartitionPairsWriter
import com.pubmatic.spark.shuffle.api.{ShuffleMapOutputWriter, ShufflePartitionWriter}
import com.pubmatic.spark.storage.{BlockId, DiskBlockObjectWriter, ShuffleBlockId}
import com.pubmatic.spark.util.{Utils => TryUtils}
    ...
    def spill(): Boolean = SPILL_LOCK.synchronized {
      if (hasSpilled) {
        ...
        logInfo(s"Task ${TaskContext.get().taskAttemptId} force spilling in-memory map to disk " +
          s"and it will release ${com.pubmatic.spark.util.Utils.bytesToString(getUsed())} memory")
        val spillFile = spillMemoryIteratorToDisk(inMemoryIterator)
