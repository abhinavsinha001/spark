...
import com.pubmatic.spark.serializer.{DeserializationStream, Serializer, SerializerManager}
import com.pubmatic.spark.storage.{BlockId, BlockManager}
import com.pubmatic.spark.util.CompletionIterator
import com.pubmatic.spark.util.collection.ExternalAppendOnlyMap.HashComparator
    ...
    def spill(): Boolean = SPILL_LOCK.synchronized {
      if (hasSpilled) {
        ...
        logInfo(s"Task ${context.taskAttemptId} force spilling in-memory map to disk and " +
          s"it will release ${com.pubmatic.spark.util.Utils.bytesToString(getUsed())} memory")
