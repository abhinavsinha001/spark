...
import com.pubmatic.spark.{HashPartitioner, Partitioner}
import com.pubmatic.spark.Partitioner._
import com.pubmatic.spark.api.java.JavaSparkContext.fakeClassTag
import com.pubmatic.spark.api.java.JavaUtils.mapAsSerializableJavaMap
import com.pubmatic.spark.api.java.function.{FlatMapFunction, Function, PairFunction}
import com.pubmatic.spark.partial.{BoundedDouble, PartialResult}
import com.pubmatic.spark.rdd.{OrderedRDDFunctions, RDD}
import com.pubmatic.spark.rdd.RDD.rddToPairRDDFunctions
...
class JavaPairRDD[K, V](val rdd: RDD[(K, V)])
  ...
  def combineByKey[C](createCombiner: Function[V, C],
                      mergeValue: function.Function2[C, V, C],
                      mergeCombiners: function.Function2[C, C, C],
   ...
   */
  def reduceByKey(partitioner: Partitioner, func: function.Function2[V, V, V]): JavaPairRDD[K, V] =
   ...
   */
  def reduceByKeyLocally(func: function.Function2[V, V, V]): java.util.Map[K, V] =
   ...
   * V.
   */
  def aggregateByKey[U](zeroValue: U, partitioner: Partitioner, seqFunc: function.Function2[U, V, U],
                        combFunc: function.Function2[U, U, U]): JavaPairRDD[K, U] = {
   ...
   */
  def aggregateByKey[U](zeroValue: U, numPartitions: Int, seqFunc: function.Function2[U, V, U],
                        combFunc: function.Function2[U, U, U]): JavaPairRDD[K, U] = {
    implicit val ctag: ClassTag[U] = fakeClassTag
   ...
   * This function can return a different result type, U, than the type of the values in this RDD,
   */
  def aggregateByKey[U](zeroValue: U, seqFunc: function.Function2[U, V, U], combFunc: function.Function2[U, U, U]):
      JavaPairRDD[K, U] = {
    implicit val ctag: ClassTag[U] = fakeClassTag
   ...
   */
  def foldByKey(zeroValue: V, partitioner: Partitioner, func: function.Function2[V, V, V])
  ...
  def foldByKey(zeroValue: V, numPartitions: Int, func: function.Function2[V, V, V]): JavaPairRDD[K, V] =
   ...
   * (e.g., Nil for list concatenation, 0 for addition, or 1 for multiplication.).
   */
  def foldByKey(zeroValue: V, func: function.Function2[V, V, V]): JavaPairRDD[K, V] =
   ...
   */
  def reduceByKey(func: function.Function2[V, V, V], numPartitions: Int): JavaPairRDD[K, V] =
  ...
  def combineByKey[C](createCombiner: Function[V, C],
                      mergeValue: function.Function2[C, V, C],
                      mergeCombiners: function.Function2[C, C, C]): JavaPairRDD[K, C] = {
    implicit val ctag: ClassTag[C] = fakeClassTag
   ...
   * parallelism level.
   */
  def reduceByKey(func: function.Function2[V, V, V]): JavaPairRDD[K, V] = {
...
object JavaPairRDD {
  ...
  private[spark]
  implicit def toScalaFunction2[T1, T2, R](fun: function.Function2[T1, T2, R]): Function2[T1, T2, R] = {
    (x: T1, x1: T2) => fun.call(x, x1)
  }

  private[spark] implicit def toScalaFunction[T, R](fun: Function[T, R]): T => R = x => fun.call(x)
