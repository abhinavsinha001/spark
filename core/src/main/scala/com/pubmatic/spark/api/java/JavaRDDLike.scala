...
import com.pubmatic.spark.api.java.JavaPairRDD._
...
import com.pubmatic.spark.api.java.function.{DoubleFlatMapFunction, DoubleFunction, FlatMapFunction, FlatMapFunction2, Function, Function2, PairFlatMapFunction, PairFunction, VoidFunction}
...
trait JavaRDDLike[T, This <: JavaRDDLike[T, This]] extends Serializable {
  ...
  /** The [[com.pubmatic.spark.SparkContext]] that this RDD was created on. */
  ...
  def mapPartitionsWithIndex[R](
                                 f: Function2[jl.Integer, JIterator[T], JIterator[R]],
   ...
   * mapping to that key.
   */
  def groupBy[U](f: Function[T, U]): JavaPairRDD[U, JIterable[T]] = {
   ...
   * mapping to that key.
   */
  def groupBy[U](f: Function[T, U], numPartitions: Int): JavaPairRDD[U, JIterable[T]] = {
    // The type parameter is U instead of K in order to work around a compiler bug; see SPARK-4459
    implicit val ctagK: ClassTag[U] = fakeClassTag
    implicit val ctagV: ClassTag[JList[T]] = fakeClassTag
   ...
   * operator.
   */
  def reduce(f: Function2[T, T, T]): T = rdd.reduce(f)

  /**
   * Reduces the elements of this RDD in a multi-level tree pattern.
   *
   * @param depth suggested depth of the tree
   * @see [[com.pubmatic.spark.api.java.JavaRDDLike#reduce]]
   */
  def treeReduce(f: Function2[T, T, T], depth: Int): T = rdd.treeReduce(f, depth)

  /**
   * `org.apache.spark.api.java.JavaRDDLike.treeReduce` with suggested depth 2.
   */
  def treeReduce(f: Function2[T, T, T]): T = treeReduce(f, 2)
   ...
   * allocation.
   */
  def aggregate[U](zeroValue: U)(seqOp: Function2[U, T, U],
                                 combOp: Function2[U, U, U]): U =
   ...
   *
   * @param depth suggested depth of the tree
   * @see [[com.pubmatic.spark.api.java.JavaRDDLike#aggregate]]
   */
  def treeAggregate[U](
                        zeroValue: U,
                        seqOp: Function2[U, T, U],
                        combOp: Function2[U, U, U],
                        depth: Int): U = {
   ...
   */
  def keyBy[U](f: Function[T, U]): JavaPairRDD[U, T] = {
    // The type parameter is U instead of K in order to work around a compiler bug; see SPARK-4459
