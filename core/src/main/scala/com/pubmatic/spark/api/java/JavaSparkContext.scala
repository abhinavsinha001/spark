...
import com.pubmatic.spark.rdd.{EmptyRDD, HadoopRDD, NewHadoopRDD}
...
/**
 * A Java-friendly version of [[com.pubmatic.spark.SparkContext]] that returns
 * [[com.pubmatic.spark.api.java.JavaRDD]]s and works with Java collections instead of Scala ones.
 *
...
class JavaSparkContext(val sc: SparkContext) extends Closeable {
  ...
  /**
   * @param conf a [[com.pubmatic.spark.SparkConf]] object specifying Spark parameters
   */
   ...
   * @param master Cluster URL to connect to (e.g. mesos://host:port, spark://host:port, local[4]).
   * @param appName A name for your application, to display on the cluster web UI
   * @param conf a [[com.pubmatic.spark.SparkConf]] object specifying other Spark parameters
   */
   ...
   * [[com.pubmatic.spark.broadcast.Broadcast]] object for reading it in distributed functions.
