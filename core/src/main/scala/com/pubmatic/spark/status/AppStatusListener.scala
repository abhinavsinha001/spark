...
import com.pubmatic.spark.internal.config.CPUS_PER_TASK
...
import com.pubmatic.spark.status.api.v1.StageStatus
import com.pubmatic.spark.storage._
import com.pubmatic.spark.ui.SparkUI
import com.pubmatic.spark.ui.scope._
...
private[spark] class AppStatusListener(
    ...
    liveJobs.remove(event.jobId).foreach { job =>
      ...
      while (it.hasNext()) {
        ...
        if (job.stageIds.contains(e.getKey()._1)) {
          ...
          if (StageStatus.PENDING.equals(stage.status) && stage.info.numTasks > 0) {
  ...
  override def onTaskEnd(event: SparkListenerTaskEnd): Unit = {
    ...
    Option(liveStages.get((event.stageId, event.stageAttemptId))).foreach { stage =>
      ...
      val removeStage =
        stage.activeTasks == 0 &&
          (StageStatus.COMPLETE.equals(stage.status) ||
            StageStatus.FAILED.equals(stage.status))
      if (removeStage) {
        update(stage, now, last = true)
      } else {
        maybeUpdate(stage, now)
  ...
  override def onStageCompleted(event: SparkListenerStageCompleted): Unit = {
    ...
    maybeStage.foreach { stage =>
      ...
      stage.status = event.stageInfo.failureReason match {
        ...
        case _ if event.stageInfo.submissionTime.isDefined => StageStatus.COMPLETE
        case _ => StageStatus.SKIPPED
      }
  ...
  private def cleanupStages(count: Long): Unit = {
    ...
    val stages = KVUtils.viewToSeq(view, countToDelete.toInt) { s =>
      s.info.status != StageStatus.ACTIVE && s.info.status != StageStatus.PENDING
    }
