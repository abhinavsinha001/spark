...
import com.pubmatic.spark.{JobExecutionStatus, SparkConf}

import com.pubmatic.spark.status.api.v1
import com.pubmatic.spark.status.api.v1.{StageStatus, TaskSorting}

import com.pubmatic.spark.ui.scope._
import com.pubmatic.spark.util.Utils
import com.pubmatic.spark.util.kvstore.{InMemoryStore, KVStore}
...
private[spark] class AppStatusStore(
  ...
  def stageList(statuses: JList[StageStatus]): Seq[v1.StageData] = {
  ...
  def taskList(
      ...
      offset: Int,
      length: Int,
      sortBy: TaskSorting): Seq[v1.TaskData] = {
    val (indexName, ascending) = sortBy match {
      case TaskSorting.ID =>
        (None, true)
      case TaskSorting.INCREASING_RUNTIME =>
        (Some(TaskIndexNames.EXEC_RUN_TIME), true)
      case TaskSorting.DECREASING_RUNTIME =>
