...
import com.pubmatic.spark.deploy.rest._
  ...
  private[deploy] def prepareSubmitEnvironment(
    ...
    if (args.isPython && deployMode == CLIENT) {
      if (args.primaryResource == PYSPARK_SHELL) {
        ...
        args.mainClass = "import com.pubmatic.spark.deploy.PythonRunner"
    ...
    if (args.isR && deployMode == CLIENT) {
      if (args.primaryResource == SPARKR_SHELL) {
        ...
        args.mainClass = "import com.pubmatic.spark.deploy.RRunner"
    ...
    if (isYarnCluster) {
      ...
      if (args.isPython) {
        ...
        childArgs += ("--class", "import com.pubmatic.spark.deploy.PythonRunner")
      } else if (args.isR) {
        val mainFile = new Path(args.primaryResource).getName
        childArgs += ("--primary-r-file", mainFile)
        childArgs += ("--class", "import com.pubmatic.spark.deploy.RRunner")
      } else {
    ...
    if (isKubernetesCluster) {
      ...
      if (args.primaryResource != SparkLauncher.NO_RESOURCE) {
        if (args.isPython) {
          ...
          childArgs ++= Array("--main-class", "import com.pubmatic.spark.deploy.PythonRunner")
        } else if (args.isR) {
          childArgs ++= Array("--primary-r-file", args.primaryResource)
          childArgs ++= Array("--main-class", "import com.pubmatic.spark.deploy.RRunner")
        }
        else {
...
object SparkSubmit extends CommandLineUtils with Logging {
  ...
  private[deploy] val YARN_CLUSTER_SUBMIT_CLASS =
    "import com.pubmatic.spark.deploy.yarn.YarnClusterApplication"
  private[deploy] val REST_CLUSTER_SUBMIT_CLASS = classOf[RestSubmissionClientApp].getName()
  private[deploy] val STANDALONE_CLUSTER_SUBMIT_CLASS = classOf[ClientApp].getName()
  private[deploy] val KUBERNETES_CLUSTER_SUBMIT_CLASS =
    "import com.pubmatic.spark.deploy.k8s.submit.KubernetesClientApplication"
