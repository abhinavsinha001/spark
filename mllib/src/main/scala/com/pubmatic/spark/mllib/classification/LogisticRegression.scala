...
package com.pubmatic.spark.mllib.classification

import com.pubmatic.spark.SparkContext
import com.pubmatic.spark.annotation.Since
import com.pubmatic.spark.ml.linalg.DenseMatrix
import com.pubmatic.spark.ml.util.Identifiable
import com.pubmatic.spark.mllib.classification.impl.GLMClassificationModel
import com.pubmatic.spark.mllib.linalg.{DenseVector, Vector, Vectors}
import com.pubmatic.spark.mllib.linalg.BLAS.dot
import com.pubmatic.spark.mllib.optimization._
import com.pubmatic.spark.mllib.pmml.PMMLExportable
import com.pubmatic.spark.mllib.regression._
import com.pubmatic.spark.mllib.util.{DataValidators, Loader, Saveable}
...
object LogisticRegressionModel extends Loader[LogisticRegressionModel] {
  ...
  override def load(sc: SparkContext, path: String): LogisticRegressionModel = {
    ...
    val classNameV1_0 = "com.pubmatic.spark.mllib.classification.LogisticRegressionModel"
  ...
  private def run(input: RDD[LabeledPoint], initialWeights: Vector, userSuppliedWeights: Boolean):
    ...
    if (numOfLinearPredictor == 1) {
      def runWithMlLogisticRegression(elasticNetParam: Double) = {
        ...
        val lr = new com.pubmatic.spark.ml.classification.LogisticRegression()
        ...
        if (userSuppliedWeights) {
          ...
          lr.setInitialModel(new com.pubmatic.spark.ml.classification.LogisticRegressionModel(uid,
