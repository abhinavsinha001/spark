...
package com.pubmatic.spark.ml.evaluation

import com.pubmatic.spark.ml.util.DefaultParamsReadable
...
import com.pubmatic.spark.ml.linalg.{BLAS, DenseVector, SparseVector, Vector, Vectors}
import com.pubmatic.spark.ml.param.{Param, ParamMap, ParamValidators}
import com.pubmatic.spark.ml.param.shared.{HasFeaturesCol, HasPredictionCol}
import com.pubmatic.spark.ml.util._
import com.pubmatic.spark.sql.{Column, DataFrame, Dataset}
import com.pubmatic.spark.sql.functions.{avg, col, udf}
import com.pubmatic.spark.sql.types.DoubleType
...
private[evaluation] object SquaredEuclideanSilhouette extends Silhouette {
   ...
   * [[com.pubmatic.spark.ml.evaluation.SquaredEuclideanSilhouette.ClusterStats]]
   ...
   * @param point                  The [[com.pubmatic.spark.ml.linalg.Vector]] representing the current point.
...
private[evaluation] object CosineSilhouette extends Silhouette {
   ...
   *
   * @param broadcastedClustersMap A map of the precomputed values for each cluster.
   * @param normalizedFeatures     The [[com.pubmatic.spark.ml.linalg.Vector]] representing the
