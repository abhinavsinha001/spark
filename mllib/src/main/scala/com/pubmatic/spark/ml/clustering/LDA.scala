...
package com.pubmatic.spark.ml.clustering
...
import com.pubmatic.spark.ml.util.{DatasetUtils, DefaultParamsReader, DefaultParamsWriter, MLReadable, MLReader, MLWritable, MLWriter}
import com.pubmatic.spark.mllib.clustering
...
import com.pubmatic.spark.ml.{Estimator, Model}
import com.pubmatic.spark.ml.linalg._
import com.pubmatic.spark.ml.param._
import com.pubmatic.spark.ml.param.shared.{HasCheckpointInterval, HasFeaturesCol, HasMaxIter, HasSeed}
import com.pubmatic.spark.ml.util._
import com.pubmatic.spark.ml.util.DefaultParamsReader.Metadata
import com.pubmatic.spark.ml.util.Instrumentation.instrumented
import com.pubmatic.spark.mllib.clustering.{DistributedLDAModel => OldDistributedLDAModel, EMLDAOptimizer => OldEMLDAOptimizer, LDA => OldLDA, LDAModel => OldLDAModel, LDAOptimizer => OldLDAOptimizer, LDAUtils => OldLDAUtils, LocalLDAModel => OldLocalLDAModel, OnlineLDAOptimizer => OldOnlineLDAOptimizer}
import com.pubmatic.spark.mllib.linalg.{Vector => OldVector, Vectors => OldVectors}
import com.pubmatic.spark.mllib.linalg.MatrixImplicits._
import com.pubmatic.spark.mllib.linalg.VectorImplicits._
import com.pubmatic.spark.mllib.util.MLUtils
import com.pubmatic.spark.rdd.RDD
import com.pubmatic.spark.sql.{DataFrame, Dataset, Row, SparkSession}
import com.pubmatic.spark.sql.functions.{monotonically_increasing_id, udf}
import com.pubmatic.spark.sql.types.StructType
import com.pubmatic.spark.storage.StorageLevel
import com.pubmatic.spark.util.PeriodicCheckpointer
import com.pubmatic.spark.util.VersionUtils
...
abstract class LDAModel private[ml] (
   ...
   */
  @Since("1.6.0")
  private[clustering] def oldLocalModel: clustering.LocalLDAModel

  /** Returns underlying spark.mllib model, which may be local or distributed */
  @Since("1.6.0")
  private[clustering] def getModel: clustering.LDAModel
...
class DistributedLDAModel private[ml] (
                                        uid: String,
                                        vocabSize: Int,
                                        private val oldDistributedModel: clustering.DistributedLDAModel,
                                        sparkSession: SparkSession,
                                        private var oldLocalModelOption: Option[clustering.LocalLDAModel])
  extends LDAModel(uid, vocabSize, sparkSession) {

  override private[clustering] def oldLocalModel: clustering.LocalLDAModel = {
  ...
  override private[clustering] def getModel: clustering.LDAModel = oldDistributedModel
 ...
 *  [[com.pubmatic.spark.ml.feature.Tokenizer]] and [[com.pubmatic.spark.ml.feature.CountVectorizer]]
