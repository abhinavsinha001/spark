...
import com.pubmatic.spark.ml.{PredictionModel, Predictor, PredictorParams}
import com.pubmatic.spark.ml.feature.{Instance, LabeledPoint}
import com.pubmatic.spark.ml.linalg.{Vector, VectorUDT}
import com.pubmatic.spark.ml.param.shared.HasRawPredictionCol
import com.pubmatic.spark.ml.util.{MetadataUtils, SchemaUtils}
...
abstract class Classifier[
   ...
   * and put it in an RDD with strong types.
   *
   * @param dataset    DataFrame with columns for labels ([[com.pubmatic.spark.sql.types.NumericType]])
   *                   and features (`Vector`).
