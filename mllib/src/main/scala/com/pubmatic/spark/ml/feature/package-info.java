 ...
 * Most feature transformers are implemented as {@link com.pubmatic.spark.ml.Transformer}s, which
 * transforms one {@link com.pubmatic.spark.sql.Dataset} into another, e.g.,
 * {@link com.pubmatic.spark.ml.feature.HashingTF}.
 * Some feature transformers are implemented as {@link com.pubmatic.spark.ml.Estimator}}s, because the
 * transformation requires some aggregated information of the dataset, e.g., document
 * frequencies in {@link com.pubmatic.spark.ml.feature.IDF}.
 * For those feature transformers, calling {@link com.pubmatic.spark.ml.Estimator#fit} is required to
 * obtain the model first, e.g., {@link com.pubmatic.spark.ml.feature.IDFModel}, in order to apply
 * transformation.
 * The transformation is usually done by appending new columns to the input
 * {@link com.pubmatic.spark.sql.Dataset}, so all input columns are carried over.
 *
 * We try to make each transformer minimal, so it becomes flexible to assemble feature
 * transformation pipelines.
 * {@link com.pubmatic.spark.ml.Pipeline} can be used to chain feature transformers, and
 * {@link com.pubmatic.spark.ml.feature.VectorAssembler} can be used to combine multiple feature
 ...
 *
 *   import com.pubmatic.spark.ml.feature.*;
