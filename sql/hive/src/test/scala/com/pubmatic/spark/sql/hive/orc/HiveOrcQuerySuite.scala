...
package com.pubmatic.spark.sql.hive.orc
...
import com.pubmatic.spark.sql.execution.datasources.orc.OrcQueryTest
import com.pubmatic.spark.sql.hive.{HiveSessionCatalog, HiveUtils}
...
class HiveOrcQuerySuite extends OrcQueryTest with TestHiveSingleton {
  ...
  test("SPARK-20728 Make ORCFileFormat configurable between sql/hive and sql/core") {
    Seq(
      ("native", classOf[com.pubmatic.spark.sql.execution.datasources.orc.OrcFileFormat]),
      ("hive", classOf[com.pubmatic.spark.sql.hive.orc.OrcFileFormat])).foreach {
      case (orcImpl, format) =>
