...
package com.pubmatic.spark.sql.hive
...
import com.pubmatic.spark.sql.catalyst.catalog.{CatalogStorageFormat, CatalogTable, CatalogTableType}
import com.pubmatic.spark.sql.execution.command.CreateTableCommand
import com.pubmatic.spark.sql.execution.datasources.{HadoopFsRelation, LogicalRelation}
import com.pubmatic.spark.sql.hive.HiveExternalCatalog._
import com.pubmatic.spark.sql.hive.test.TestHiveSingleton
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.internal.StaticSQLConf._
...
class MetastoreDataSourcesSuite extends QueryTest with SQLTestUtils with TestHiveSingleton {
    ...
    withTable("jsonTable") {
      sql(
        s"""CREATE TABLE jsonTable
           |USING com.pubmatic.spark.sql.json.DefaultSource
           |OPTIONS (
           |  path '$jsonFilePath'
           |)
         """.stripMargin)
    ...
    withTempPath { path =>
      ...
      withTable("savedJsonTable") {
        ...
        withSQLConf(SQLConf.DEFAULT_DATA_SOURCE_NAME.key -> "not a source name") {
          df.write
            .format("com.pubmatic.spark.sql.json")
  ...
  test("append a table using the same formats but different names") {
    ...
    withTable("appendParquet") {
      ...
      createDF(10, 19).write.mode(SaveMode.Append).format("com.pubmatic.spark.sql.parquet")
    ...
    withTable("appendParquet") {
      createDF(0, 9).write.format("com.pubmatic.spark.sql.parquet").saveAsTable("appendParquet")
    ...
    withTable("appendParquet") {
      createDF(0, 9).write.format("com.pubmatic.spark.sql.parquet.DefaultSource")
        .saveAsTable("appendParquet")
      createDF(10, 19).write.mode(SaveMode.Append)
        .format("com.pubmatic.spark.sql.execution.datasources.parquet.DefaultSource")
        .saveAsTable("appendParquet")
      checkAnswer(
        sql("SELECT p.c1, p.c2 FROM appendParquet p WHERE p.c1 > 5"),
        (6 to 19).map(i => Row(i, s"str$i")))
  ...
  test("append a table with file source V2 provider using the v1 file format") {
    ...
    withTable("appendCSV") {
      createDF(0, 9)
        .write
        .mode(SaveMode.Append)
        .format("com.pubmatic.spark.sql.execution.datasources.v2.csv.CSVDataSourceV2")
        .saveAsTable("appendCSV")
      createDF(10, 19)
        .write
        .mode(SaveMode.Append)
        .format("com.pubmatic.spark.sql.execution.datasources.csv.CSVFileFormat")
        .saveAsTable("appendCSV")
      checkAnswer(
