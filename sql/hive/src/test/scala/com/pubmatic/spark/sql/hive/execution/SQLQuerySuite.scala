...
import com.pubmatic.spark.{SparkException, TestUtils}
import com.pubmatic.spark.sql._
import com.pubmatic.spark.sql.catalyst.TableIdentifier
import com.pubmatic.spark.sql.catalyst.analysis.{EliminateSubqueryAliases, FunctionRegistry}
import com.pubmatic.spark.sql.catalyst.catalog.{CatalogTableType, CatalogUtils, HiveTableRelation}
import com.pubmatic.spark.sql.catalyst.parser.ParseException
import com.pubmatic.spark.sql.catalyst.plans.logical.{LogicalPlan, SubqueryAlias}
import com.pubmatic.spark.sql.execution.command.{FunctionsCommand, LoadDataCommand}
import com.pubmatic.spark.sql.execution.datasources.{HadoopFsRelation, LogicalRelation}
import com.pubmatic.spark.sql.functions._
import com.pubmatic.spark.sql.hive.{HiveExternalCatalog, HiveUtils}
import com.pubmatic.spark.sql.hive.test.{HiveTestJars, TestHiveSingleton}
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.internal.StaticSQLConf.GLOBAL_TEMP_DATABASE
import com.pubmatic.spark.sql.test.SQLTestUtils
...
class SQLQuerySuite extends QueryTest with SQLTestUtils with TestHiveSingleton {
  ...
  test("describe functions - built-in functions") {
    ...
    checkKeywordsExist(sql("describe functioN  `~`"),
      "Function: ~",
      "Class: com.pubmatic.spark.sql.catalyst.expressions.BitwiseNot",
  ...
  test("describe functions - user defined functions") {
    withUserDefinedFunction("udtf_count" -> false) {
      ...
      checkKeywordsExist(sql("describe function udtf_count"),
        ...
        "Class: com.pubmatic.spark.sql.hive.execution.GenericUDTFCount2",
        "Usage: N/A")
  ...
  test("run sql directly on files - parquet") {
    ...
    withTempPath(f => {
      ...
      checkAnswer(sql(s"select id from Parquet.`${f.getCanonicalPath}`"),
        df)
      checkAnswer(sql(s"select id from `com.pubmatic.spark.sql.parquet`.`${f.getCanonicalPath}`"),
        df)
  ...
  test("run sql directly on files - orc") {
    ...
    withTempPath(f => {
      ...
      checkAnswer(sql(s"select id from ORC.`${f.getCanonicalPath}`"),
        df)
      checkAnswer(sql(s"select id from `com.pubmatic.spark.sql.hive.orc`.`${f.getCanonicalPath}`"),
        df)
    ...
    withUserDefinedFunction("udtf_stack1" -> true, "udtf_stack2" -> true) {
      sql(
        s"""
           ...
           |AS 'com.pubmatic.spark.sql.hive.execution.UDTFStack'
      ...
      sql(
        s"""
           ...
           |AS 'com.pubmatic.spark.sql.hive.execution.UDTFStack2'
           |USING JAR '${hiveContext.getHiveFile("SPARK-21101-1.0.jar").toURI}'
        """.stripMargin)
      val e = intercept[com.pubmatic.spark.sql.AnalysisException] {
