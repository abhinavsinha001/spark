...
package com.pubmatic.spark.sql.hive.test
...
import com.pubmatic.spark.sql.{DataFrame, Dataset, SparkSession, SQLContext}
import com.pubmatic.spark.sql.catalyst.analysis.UnresolvedRelation
import com.pubmatic.spark.sql.catalyst.catalog.ExternalCatalogWithListener
import com.pubmatic.spark.sql.catalyst.optimizer.ConvertToLocalRelation
import com.pubmatic.spark.sql.catalyst.plans.logical.{LogicalPlan, OneRowRelation}
import com.pubmatic.spark.sql.execution.{QueryExecution, SQLExecution}
import com.pubmatic.spark.sql.execution.command.CacheTableCommand
import com.pubmatic.spark.sql.hive._
import com.pubmatic.spark.sql.hive.client.HiveClient
import com.pubmatic.spark.sql.internal.{SessionState, SharedState, SQLConf, WithTestConf}
import com.pubmatic.spark.sql.internal.StaticSQLConf.{CATALOG_IMPLEMENTATION, WAREHOUSE_PATH}
import com.pubmatic.spark.util.{ShutdownHookManager, Utils}
    ...
    new SparkContext(
      ...
      new SparkConf()
        ...
        .set(HiveUtils.HIVE_METASTORE_BARRIER_PREFIXES.key,
          "com.pubmatic.spark.sql.hive.execution.PairSerDe")
    ...
    val hiveQTestUtilTables: Seq[TestTable] = Seq(
      ...
      TestTable("src_thrift", () => {
        ...
        s"""
           ...
           |  'serialization.class'='com.pubmatic.spark.sql.hive.test.Complex',
