...
import com.pubmatic.spark.sql.catalyst.analysis.{NoSuchPartitionException, TableAlreadyExistsException}
import com.pubmatic.spark.sql.catalyst.catalog._
import com.pubmatic.spark.sql.catalyst.parser.ParseException
import com.pubmatic.spark.sql.execution.command.{DDLSuite, DDLUtils}
import com.pubmatic.spark.sql.functions._
import com.pubmatic.spark.sql.hive.HiveExternalCatalog
import com.pubmatic.spark.sql.hive.HiveUtils.{CONVERT_METASTORE_ORC, CONVERT_METASTORE_PARQUET}
import com.pubmatic.spark.sql.hive.orc.OrcFileOperator
import com.pubmatic.spark.sql.hive.test.TestHiveSingleton
import com.pubmatic.spark.sql.internal.{HiveSerDe, SQLConf}
import com.pubmatic.spark.sql.internal.SQLConf.ORC_IMPLEMENTATION
...
class HiveCatalogedDDLSuite extends DDLSuite with TestHiveSingleton with BeforeAndAfterEach {
  ...
  test("Create Table LIKE USING Hive built-in ORC in Hive catalog") {
    ...
    withTable("s", "t") {
      ...
      assert(table.provider == Some("com.pubmatic.spark.sql.hive.orc"))
    }
  }
}
...
class HiveDDLSuite
  ...
  test("datasource and statistics table property keys are not allowed") {
    import com.pubmatic.spark.sql.hive.HiveExternalCatalog.DATASOURCE_PREFIX
    import com.pubmatic.spark.sql.hive.HiveExternalCatalog.STATISTICS_PREFIX
  ...
  test("SPARK-21216: join with a streaming DataFrame") {
    import com.pubmatic.spark.sql.execution.streaming.MemoryStream
    import testImplicits._
