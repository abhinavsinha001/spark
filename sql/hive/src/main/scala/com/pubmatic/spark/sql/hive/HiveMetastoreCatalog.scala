...
import com.pubmatic.spark.sql.execution.datasources.parquet.{ParquetFileFormat, ParquetOptions}
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.internal.SQLConf.HiveCaseSensitiveInferenceMode._
    ...
    if (serde.contains("parquet")) {
      ...
      if (SQLConf.get.getConf(SQLConf.ORC_IMPLEMENTATION) == "native") {
        convertToLogicalRelation(
          relation,
          options,
          classOf[com.pubmatic.spark.sql.execution.datasources.orc.OrcFileFormat],
          "orc")
      } else {
        convertToLogicalRelation(
          relation,
          options,
          classOf[com.pubmatic.spark.sql.hive.orc.OrcFileFormat],
          "orc")
      }
    }
  }
