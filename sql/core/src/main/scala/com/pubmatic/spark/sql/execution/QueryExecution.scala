...
import com.pubmatic.spark.sql.catalyst.{InternalRow, QueryPlanningTracker}
import com.pubmatic.spark.sql.catalyst.analysis.UnsupportedOperationChecker
import com.pubmatic.spark.sql.catalyst.expressions.codegen.ByteCodeStats
import com.pubmatic.spark.sql.catalyst.plans.QueryPlan
import com.pubmatic.spark.sql.catalyst.plans.logical.{LogicalPlan, ReturnAnswer}
import com.pubmatic.spark.sql.catalyst.rules.Rule
import com.pubmatic.spark.sql.catalyst.util.StringUtils.PlanStringConcat
import com.pubmatic.spark.sql.catalyst.util.truncatedString
import com.pubmatic.spark.sql.dynamicpruning.PlanDynamicPruningFilters
import com.pubmatic.spark.sql.execution.adaptive.InsertAdaptiveSparkPlan
import com.pubmatic.spark.sql.execution.exchange.{EnsureRequirements, ReuseExchange}
...
class QueryExecution(
  ...
  // scalastyle:on
    ...
    def codegen(): Unit = {
      ...
      println(com.pubmatic.spark.sql.execution.debug.codegenString(executedPlan))
    ...
    def codegenToSeq(): Seq[(String, String, ByteCodeStats)] = {
      com.pubmatic.spark.sql.execution.debug.codegenStringSeq(executedPlan)
    }
    ...
    def toFile(path: String, maxFields: Int = Int.MaxValue): Unit = {
      ...
      try {
        ...
        writer.write("\n== Whole Stage Codegen ==\n")
        com.pubmatic.spark.sql.execution.debug.writeCodegen(writer.write, executedPlan)
      } finally {
        writer.close()
      }
    }
