...
import com.pubmatic.spark.sql.catalyst.TableIdentifier
import com.pubmatic.spark.sql.catalyst.analysis.{EliminateSubqueryAliases, NoSuchTableException, UnresolvedRelation}
import com.pubmatic.spark.sql.catalyst.catalog._
import com.pubmatic.spark.sql.catalyst.expressions.Literal
import com.pubmatic.spark.sql.catalyst.plans.logical.{AppendData, CreateTableAsSelect, InsertIntoStatement, LogicalPlan, OverwriteByExpression, OverwritePartitionsDynamic, ReplaceTableAsSelect}
import com.pubmatic.spark.sql.catalyst.util.CaseInsensitiveMap
import com.pubmatic.spark.sql.connector.catalog.{CatalogPlugin, Identifier, SupportsWrite, TableCatalog, TableProvider, V1Table}
import com.pubmatic.spark.sql.connector.catalog.TableCapability._
import com.pubmatic.spark.sql.connector.expressions.{BucketTransform, FieldReference, IdentityTransform, LiteralValue, Transform}
import com.pubmatic.spark.sql.execution.SQLExecution
import com.pubmatic.spark.sql.execution.command.DDLUtils
import com.pubmatic.spark.sql.execution.datasources.{CreateTable, DataSource, DataSourceUtils, LogicalRelation}
import com.pubmatic.spark.sql.execution.datasources.v2._
import com.pubmatic.spark.sql.internal.SQLConf.PartitionOverwriteMode
import com.pubmatic.spark.sql.sources.BaseRelation
import com.pubmatic.spark.sql.types.{IntegerType, StructType}
...
final class DataFrameWriter[T] private[sql](ds: Dataset[T]) {
  ...
  def save(): Unit = {
    ...
    if (maybeV2Provider.isDefined) {
      ...
      import com.pubmatic.spark.sql.execution.datasources.v2.DataSourceV2Implicits._
