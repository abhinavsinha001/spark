...
import com.pubmatic.spark.internal.config.ConfigEntry
...
import com.pubmatic.spark.sql.execution.command.ShowTablesCommand
import com.pubmatic.spark.sql.internal.{SQLConf, SessionState, SharedState}
import com.pubmatic.spark.sql.sources.BaseRelation
import com.pubmatic.spark.sql.streaming.{DataStreamReader, StreamingQueryManager}
...
class SQLContext private[sql](val sparkSession: SparkSession)
   ...
   *  import com.pubmatic.spark.sql.types._
   *  val sqlContext = new com.pubmatic.spark.sql.SQLContext(sc)
   *
   *  val schema =
   *    StructType(
   ...
   * [[com.pubmatic.spark.sql.streaming.StreamingQuery StreamingQueries]] active on `this` context.
   *
   * @since 2.0.0
   */
