...
import com.pubmatic.spark.sql.catalyst.analysis.{CannotReplaceMissingTableException, NoSuchTableException, TableAlreadyExistsException}
import com.pubmatic.spark.sql.catalyst.expressions.{Attribute, Bucket, Days, Hours, Literal, Months, Years}
import com.pubmatic.spark.sql.catalyst.plans.logical.{AppendData, CreateTableAsSelect, LogicalPlan, OverwriteByExpression, OverwritePartitionsDynamic, ReplaceTableAsSelect}
import com.pubmatic.spark.sql.connector.catalog.TableCatalog
import com.pubmatic.spark.sql.connector.expressions.{LogicalExpressions, NamedReference, Transform}
import com.pubmatic.spark.sql.execution.SQLExecution
import com.pubmatic.spark.sql.execution.datasources.v2.DataSourceV2Relation
import com.pubmatic.spark.sql.types.IntegerType

/**
 * Interface used to write a [[com.pubmatic.spark.sql.Dataset]] to external storage using the v2 API.
 *
 * @since 3.0.0
 */
@Experimental
final class DataFrameWriterV2[T] private[sql](table: String, ds: Dataset[T])
  ...
  import com.pubmatic.spark.sql.connector.catalog.CatalogV2Implicits._
  import com.pubmatic.spark.sql.connector.catalog.CatalogV2Util._
   ...
   * [[com.pubmatic.spark.sql.catalyst.analysis.NoSuchTableException]]. The data frame will be
   * validated to ensure it is compatible with the existing table.
   *
   * @throws com.pubmatic.spark.sql.catalyst.analysis.NoSuchTableException If the table does not exist
   */
   ...
   *
   * If the output table does not exist, this operation will fail with
   * [[com.pubmatic.spark.sql.catalyst.analysis.NoSuchTableException]].
...
trait CreateTableWriter[T] extends WriteConfigMethods[CreateTableWriter[T]] {
   ...
   * [[com.pubmatic.spark.sql.catalyst.analysis.TableAlreadyExistsException]].
   *
   * @throws com.pubmatic.spark.sql.catalyst.analysis.TableAlreadyExistsException
   ...
   *
   * If the output table does not exist, this operation will fail with
   * [[com.pubmatic.spark.sql.catalyst.analysis.CannotReplaceMissingTableException]].
   *
   * @throws com.pubmatic.spark.sql.catalyst.analysis.CannotReplaceMissingTableException
   *         If the table already exists
   */
