...
import com.pubmatic.spark.{broadcast, TaskContext}
import com.pubmatic.spark.rdd.RDD
import com.pubmatic.spark.sql.catalyst.InternalRow
import com.pubmatic.spark.sql.catalyst.expressions.{Attribute, SortOrder, SpecializedGetters, UnsafeProjection}
import com.pubmatic.spark.sql.catalyst.expressions.codegen._
import com.pubmatic.spark.sql.catalyst.expressions.codegen.Block._
import com.pubmatic.spark.sql.catalyst.plans.physical.Partitioning
import com.pubmatic.spark.sql.catalyst.rules.Rule
import com.pubmatic.spark.sql.execution.metric.{SQLMetric, SQLMetrics}
import com.pubmatic.spark.sql.execution.vectorized.{OffHeapColumnVector, OnHeapColumnVector, WritableColumnVector}
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.types._
import com.pubmatic.spark.sql.vectorized.{ColumnarBatch, ColumnVector}
 ...
 * [[com.pubmatic.spark.sql.execution.python.ArrowEvalPythonExec]] and
...
case class ColumnarToRowExec(child: SparkPlan) extends UnaryExecNode with CodegenSupport {
   ...
   * This produces an [[com.pubmatic.spark.sql.catalyst.expressions.UnsafeRow]] for each row in
   * each batch.
   */
 ...
 * [[com.pubmatic.spark.sql.execution.arrow.ArrowWriter]]. That code is more specialized
 ...
 * [[com.pubmatic.spark.sql.execution.vectorized.ColumnVectorUtils.populate()]] and
 * [[com.pubmatic.spark.sql.execution.vectorized.ColumnVectorUtils.toBatch()]] toBatch is only ever
