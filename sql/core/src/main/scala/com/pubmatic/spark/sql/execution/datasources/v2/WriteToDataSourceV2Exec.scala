...
import com.pubmatic.spark.{SparkEnv, SparkException, TaskContext}
import com.pubmatic.spark.executor.CommitDeniedException
...
import com.pubmatic.spark.sql.connector.catalog.{Identifier, StagedTable, StagingTableCatalog, SupportsWrite, TableCatalog}
import com.pubmatic.spark.sql.connector.expressions.Transform
import com.pubmatic.spark.sql.connector.write.{BatchWrite, DataWriterFactory, PhysicalWriteInfoImpl, SupportsDynamicOverwrite, SupportsOverwrite, SupportsTruncate, V1WriteBuilder, WriteBuilder, WriterCommitMessage}
import com.pubmatic.spark.sql.execution.{SparkPlan, UnaryExecNode}
import com.pubmatic.spark.sql.sources.{AlwaysTrue, Filter}
import com.pubmatic.spark.sql.util.CaseInsensitiveStringMap
import com.pubmatic.spark.util.{LongAccumulator, Utils}

/**
 * Deprecated logical plan for writing data into data source v2. This is being replaced by more
 * specific logical plans, like [[com.pubmatic.spark.sql.catalyst.plans.logical.AppendData]].
 */
...
case class CreateTableAsSelectExec(
  ...
  import com.pubmatic.spark.sql.connector.catalog.CatalogV2Implicits.IdentifierHelper
