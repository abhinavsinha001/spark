...
import com.pubmatic.spark.sql.catalyst.catalog.CatalogStorageFormat
...
object HiveSerDe {
  ...
  def sourceToSerDe(source: String): Option[HiveSerDe] = {
    val key = source.toLowerCase(Locale.ROOT) match {
      case s if s.startsWith("com.pubmatic.spark.sql.parquet") => "parquet"
      case s if s.startsWith("com.pubmatic.spark.sql.execution.datasources.parquet") => "parquet"
      case s if s.startsWith("com.pubmatic.spark.sql.orc") => "orc"
      case s if s.startsWith("com.pubmatic.spark.sql.hive.orc") => "orc"
      case s if s.startsWith("com.pubmatic.spark.sql.execution.datasources.orc") => "orc"
