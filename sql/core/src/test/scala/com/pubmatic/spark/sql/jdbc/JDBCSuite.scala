...
package com.pubmatic.spark.sql.jdbc
...
import com.pubmatic.spark.sql.{AnalysisException, DataFrame, QueryTest, Row}
import com.pubmatic.spark.sql.catalyst.TableIdentifier
import com.pubmatic.spark.sql.catalyst.parser.CatalystSqlParser
import com.pubmatic.spark.sql.catalyst.util.{CaseInsensitiveMap, DateTimeTestUtils}
import com.pubmatic.spark.sql.execution.DataSourceScanExec
import com.pubmatic.spark.sql.execution.command.{ExplainCommand, ShowCreateTableCommand}
import com.pubmatic.spark.sql.execution.datasources.LogicalRelation
import com.pubmatic.spark.sql.execution.datasources.jdbc.{JDBCOptions, JDBCPartition, JDBCRDD, JDBCRelation, JdbcUtils}
import com.pubmatic.spark.sql.execution.metric.InputOutputMetricsHelper
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.sources._
import com.pubmatic.spark.sql.test.SharedSparkSession
...
class JDBCSuite extends QueryTest
  ...
  private def checkPushdown(df: DataFrame): DataFrame = {
    ...
    assert(parentPlan.isInstanceOf[com.pubmatic.spark.sql.execution.WholeStageCodegenExec])
    val node = parentPlan.asInstanceOf[com.pubmatic.spark.sql.execution.WholeStageCodegenExec]
    assert(node.child.isInstanceOf[com.pubmatic.spark.sql.execution.DataSourceScanExec])
  ...
  private def checkNotPushdown(df: DataFrame): DataFrame = {
    ...
    assert(parentPlan.isInstanceOf[com.pubmatic.spark.sql.execution.WholeStageCodegenExec])
    val node = parentPlan.asInstanceOf[com.pubmatic.spark.sql.execution.WholeStageCodegenExec]
    assert(node.child.isInstanceOf[com.pubmatic.spark.sql.execution.FilterExec])
    df
  }
  ...
  test("SELECT COUNT(1) WHERE (predicates)") {
    ...
    // com.pubmatic.spark.sql.sources.interfaces is not given in logical plans, but some filters
  ...
  test("Remap types via JdbcDialects") {
    ...
    assert(df.schema.filter(_.dataType != com.pubmatic.spark.sql.types.StringType).isEmpty)
    val rows = df.collect()
