...
import com.pubmatic.spark.{SparkFunSuite, TaskContext}
import com.pubmatic.spark.internal.Logging
import com.pubmatic.spark.rdd.RDD
import com.pubmatic.spark.sql.catalyst.{FunctionIdentifier, TableIdentifier}
import com.pubmatic.spark.sql.catalyst.expressions._
import com.pubmatic.spark.sql.catalyst.parser.{CatalystSqlParser, ParserInterface}
import com.pubmatic.spark.sql.catalyst.plans.logical.{LocalRelation, LogicalPlan, UnresolvedHint}
import com.pubmatic.spark.sql.catalyst.rules.Rule
import com.pubmatic.spark.sql.execution._
import com.pubmatic.spark.sql.execution.vectorized.OnHeapColumnVector
import com.pubmatic.spark.sql.internal.SQLConf.COLUMN_BATCH_SIZE
import com.pubmatic.spark.sql.internal.StaticSQLConf.SPARK_SESSION_EXTENSIONS
import com.pubmatic.spark.sql.types.{DataType, Decimal, IntegerType, LongType, Metadata, StructType}
import com.pubmatic.spark.sql.vectorized.{ColumnarArray, ColumnarBatch, ColumnarMap, ColumnVector}
...
trait ColumnarExpression extends Expression with Serializable {
   ...
   * [[com.pubmatic.spark.sql.vectorized.ColumnarBatch]]. The result of
   * calling this may be a single [[com.pubmatic.spark.sql.vectorized.ColumnVector]] or a scalar
   * value. Scalar values typically happen if they are a part of the expression i.e. col("a") + 100.
   * In this case the 100 is a [[com.pubmatic.spark.sql.catalyst.expressions.Literal]] that
   * [[com.pubmatic.spark.sql.catalyst.expressions.Add]] would have to be able to handle.
   *
   * By convention any [[com.pubmatic.spark.sql.vectorized.ColumnVector]] returned by [[columnarEval]]
   * is owned by the caller and will need to be closed by them. This can happen by putting it into
   * a [[com.pubmatic.spark.sql.vectorized.ColumnarBatch]] and closing the batch or by closing the
