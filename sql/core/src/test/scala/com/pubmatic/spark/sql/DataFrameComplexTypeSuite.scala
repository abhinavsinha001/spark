...
import com.pubmatic.spark.sql.catalyst.DefinedByConstructorParams
import com.pubmatic.spark.sql.catalyst.expressions.Expression
import com.pubmatic.spark.sql.catalyst.expressions.objects.MapObjects
...
import com.pubmatic.spark.sql.types.ArrayType
...
class DataFrameComplexTypeSuite extends QueryTest with SharedSparkSession {
  ...
  test("SPARK-12477 accessing null element in array field") {
    ...
    assert(nullStringRow == com.pubmatic.spark.sql.Row(null))
    val nullIntRow = df.selectExpr("i[1]").collect()(0)
    assert(nullIntRow == com.pubmatic.spark.sql.Row(null))
  }
