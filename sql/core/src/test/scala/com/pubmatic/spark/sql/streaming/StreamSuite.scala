...
import com.pubmatic.spark.{SparkConf, SparkContext, TaskContext, TestUtils}
import com.pubmatic.spark.scheduler.{SparkListener, SparkListenerJobStart}
import com.pubmatic.spark.sql._
import com.pubmatic.spark.sql.catalyst.plans.logical.Range
import com.pubmatic.spark.sql.catalyst.streaming.InternalOutputModes
import com.pubmatic.spark.sql.catalyst.util.DateTimeUtils
import com.pubmatic.spark.sql.execution.command.ExplainCommand
import com.pubmatic.spark.sql.execution.streaming._
import com.pubmatic.spark.sql.execution.streaming.sources.{ContinuousMemoryStream, MemorySink}
import com.pubmatic.spark.sql.execution.streaming.state.{StateStore, StateStoreConf, StateStoreId, StateStoreProvider}
import com.pubmatic.spark.sql.functions._
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.sources.StreamSourceProvider
import com.pubmatic.spark.sql.streaming.util.StreamManualClock
import com.pubmatic.spark.sql.types.{IntegerType, StructField, StructType}
...
class StreamSuite extends StreamTest {
  ...
  test("codegen-microbatch") {
    ...
    try {
      import com.pubmatic.spark.sql.execution.debug._
  ...
  testQuietly("custom state store provider read from offset log") {
    ...
    val providerConf1 = SQLConf.STATE_STORE_PROVIDER_CLASS.key ->
      "com.pubmatic.spark.sql.execution.streaming.state.HDFSBackedStateStoreProvider"
