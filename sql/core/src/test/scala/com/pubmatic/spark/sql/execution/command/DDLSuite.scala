...
import com.pubmatic.spark.sql.{AnalysisException, QueryTest, Row, SaveMode}
import com.pubmatic.spark.sql.catalyst.TableIdentifier
import com.pubmatic.spark.sql.catalyst.analysis.{FunctionRegistry, NoSuchDatabaseException, NoSuchPartitionException, NoSuchTableException, TempTableAlreadyExistsException}
import com.pubmatic.spark.sql.catalyst.catalog._
import com.pubmatic.spark.sql.catalyst.catalog.CatalogTypes.TablePartitionSpec
...
import com.pubmatic.spark.sql.test.{SharedSparkSession, SQLTestUtils}
...
class InMemoryCatalogedDDLSuite extends DDLSuite with SharedSparkSession {
  ...
  protected override def generateTable(
    ...
    CatalogTable(
      ...
      createTime = 0L,
      createVersion = com.pubmatic.spark.SPARK_VERSION,
  ...
  test("throw exception if Create Table LIKE USING Hive built-in ORC in in-memory catalog") {
    ...
    withTable("s", "t") {
      ...
      val e = intercept[AnalysisException] {
        sql("CREATE TABLE t LIKE s USING com.pubmatic.spark.sql.hive.orc")
      }.getMessage
...
abstract class DDLSuite extends QueryTest with SQLTestUtils {
    ...
    withResourceTempPath("test-data/cars.csv") { tmpFile =>
      withTempView("testview") {
        sql(s"CREATE OR REPLACE TEMPORARY VIEW testview (c1 String, c2 String)  USING " +
          "com.pubmatic.spark.sql.execution.datasources.csv.CSVFileFormat  " +
        ...
        intercept[TempTableAlreadyExistsException] {
          sql(
            s"""
               ...
               |USING com.pubmatic.spark.sql.execution.datasources.csv.CSVFileFormat
    ...
    withTempView("tab1") {
      sql(
        """
          ...
          |USING com.pubmatic.spark.sql.sources.DDLScanSource
          |OPTIONS (
          |  From '1',
          |  To '10',
          |  Table 'test1'
  ...
  test("describe function") {
    checkAnswer(
      ...
      Row("Class: com.pubmatic.spark.sql.catalyst.expressions.Logarithm") ::
        Row("Function: log") ::
    ...
    checkAnswer(
      ...
      Row("Class: com.pubmatic.spark.sql.catalyst.expressions.Or") ::
        Row("Function: or") ::
        Row("Usage: expr1 or expr2 - Logical OR.") :: Nil
    )
    checkAnswer(
      sql("DESCRIBE FUNCTION !"),
      Row("Class: com.pubmatic.spark.sql.catalyst.expressions.Not") ::
        Row("Function: !") ::
    ...
    checkAnswer(
      sql("DESCRIBE FUNCTION +"),
      Row("Class: com.pubmatic.spark.sql.catalyst.expressions.Add") ::
        Row("Function: +") ::
    ...
    checkAnswer(
      sql("DESCRIBE FUNCTION <"),
      Row("Class: com.pubmatic.spark.sql.catalyst.expressions.LessThan") ::
        Row("Function: <") ::
    ...
    checkAnswer(
      ...
      Row("Class: com.pubmatic.spark.sql.catalyst.expressions.Concat") ::
        Row("Function: concat") ::
    ...
    checkAnswer(
      ...
      Row("Class: com.pubmatic.spark.sql.catalyst.expressions.BitwiseXor") ::
        Row(
          """Extended Usage:
            |    Examples:
            |      > SELECT 3 ^ 5;
  ...
  val supportedNativeFileFormatsForAlterTableAddColumns = Seq("csv", "json", "parquet",
    "com.pubmatic.spark.sql.execution.datasources.csv.CSVFileFormat",
    "com.pubmatic.spark.sql.execution.datasources.json.JsonFileFormat",
    "com.pubmatic.spark.sql.execution.datasources.parquet.ParquetFileFormat")
