...
package com.pubmatic.spark.sql.streaming.test
...
import com.pubmatic.spark.sql.sources.{StreamSinkProvider, StreamSourceProvider}
import com.pubmatic.spark.sql.streaming.{OutputMode, StreamingQuery, StreamingQueryException, StreamTest}
import com.pubmatic.spark.sql.streaming.Trigger._
...
class DataStreamReaderWriterSuite extends StreamTest with BeforeAndAfter {
  ...
  test("write cannot be called on streaming datasets") {
    val e = intercept[AnalysisException] {
      spark.readStream
        .format("com.pubmatic.spark.sql.streaming.test")
        .load()
        .write
        .save()
    }
  ...
  test("resolve full class") {
    spark.readStream
      .format("com.pubmatic.spark.sql.streaming.test.DefaultSource")
      .load()
      .writeStream
      .format("com.pubmatic.spark.sql.streaming.test")
      .option("checkpointLocation", newMetadataDir)
  ...
  test("source metadataPath") {
    ...
    verify(LastOptions.mockStreamSourceProvider).createSource(
      ...
      meq(None),
      meq("com.pubmatic.spark.sql.streaming.test"),
      meq(Map.empty))
