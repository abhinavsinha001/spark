...
package com.pubmatic.spark.sql.test
...
import com.pubmatic.spark.internal.io.HadoopMapReduceCommitProtocol
...
import com.pubmatic.spark.sql.catalyst.plans.logical.{AppendData, LogicalPlan, OverwriteByExpression}
import com.pubmatic.spark.sql.execution.QueryExecution
import com.pubmatic.spark.sql.execution.datasources.DataSourceUtils
import com.pubmatic.spark.sql.execution.datasources.noop.NoopDataSource
import com.pubmatic.spark.sql.execution.datasources.parquet.SpecificParquetRecordReaderBase
...
import com.pubmatic.spark.sql.util.QueryExecutionListener
...
class DataFrameReaderWriterSuite extends QueryTest with SharedSparkSession with BeforeAndAfter {
  ...
  test("resolve default source without extending SchemaRelationProvider") {
    spark.read
      .format("com.pubmatic.spark.sql.test.DefaultSourceWithoutUserSpecifiedSchema")
      .load()
      .write
      .format("com.pubmatic.spark.sql.test.DefaultSourceWithoutUserSpecifiedSchema")
      .save()
  ...
  test("resolve full class") {
    spark.read
      .format("com.pubmatic.spark.sql.test.DefaultSource")
      .load()
      .write
      .format("com.pubmatic.spark.sql.test")
      .save()
  ...
  test("load API") {
    ...
    spark.read.format("com.pubmatic.spark.sql.test").load(dir)
    spark.read.format("com.pubmatic.spark.sql.test").load(dir, dir, dir)
    spark.read.format("com.pubmatic.spark.sql.test").load(Seq(dir, dir): _*)
    Option(dir).map(spark.read.format("com.pubmatic.spark.sql.test").load)
  }
  ...
  test("read a data source that does not extend SchemaRelationProvider") {
    ...
    assert(e.getMessage.contains(
      "com.pubmatic.spark.sql.sources.SimpleScanSource does not allow user-specified schemas"))
  }
  ...
  test("read a data source that does not extend RelationProvider") {
    val dfReader = spark.read
      ...
      .format("com.pubmatic.spark.sql.sources.AllDataTypesScanSource")
  ...
  test("SPARK-18510: use user specified types for partition columns in file sources") {
    import com.pubmatic.spark.sql.functions.udf
    withTempDir { src =>
  ...
  test("SPARK-18912: number of columns mismatch for non-file-based data source table") {
    withTable("t") {
      sql("CREATE TABLE t USING com.pubmatic.spark.sql.test.DefaultSource")
