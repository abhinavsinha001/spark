...
package com.pubmatic.spark.sql.connector
...
import com.pubmatic.spark.sql.connector.catalog.{SupportsRead, Table, TableCapability, TableProvider}
import com.pubmatic.spark.sql.connector.catalog.TableCapability._
import com.pubmatic.spark.sql.connector.read._
import com.pubmatic.spark.sql.connector.read.partitioning.{ClusteredDistribution, Distribution, Partitioning}
import com.pubmatic.spark.sql.execution.datasources.v2.{BatchScanExec, DataSourceV2Relation, DataSourceV2ScanRelation}
import com.pubmatic.spark.sql.execution.exchange.{Exchange, ShuffleExchangeExec}
import com.pubmatic.spark.sql.execution.vectorized.OnHeapColumnVector
import com.pubmatic.spark.sql.functions._
import com.pubmatic.spark.sql.sources.{Filter, GreaterThan}
...
import test.org.apache.spark.sql.connector.{JavaAdvancedDataSourceV2, JavaColumnarDataSourceV2, JavaPartitionAwareDataSource, JavaReportStatisticsDataSource, JavaSchemaRequiredDataSource, JavaSimpleDataSourceV2}
...
class DataSourceV2Suite extends QueryTest with SharedSparkSession {
  ...
  test("partitioning reporting") {
    import com.pubmatic.spark.sql.functions.{count, sum}
  ...
  test("simple writable data source") {
    ...
    Seq(classOf[SimpleWritableDataSource]).foreach { cls =>
      withTempPath { file =>
        ...
        // test transaction
        val failingUdf = com.pubmatic.spark.sql.functions.udf {
          var count = 0
          (id: Long) => {
            if (count > 5) {
