...
import com.pubmatic.spark.sql.execution.streaming.FileStreamSource.{FileEntry, SeenFilesMap, SourceFileArchiver}
import com.pubmatic.spark.sql.execution.streaming.sources.MemorySink
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.streaming.ExistsThrowsExceptionFileSystem._
...
import com.pubmatic.spark.sql.types.{StructType, _}
  ...
  test("FileStreamSource schema: orc, existing files, no schema") {
    withTempDir { src =>
      Seq("a", "b", "c").toDS().as("userColumn").toDF().write
        .mode(com.pubmatic.spark.sql.SaveMode.Overwrite)
