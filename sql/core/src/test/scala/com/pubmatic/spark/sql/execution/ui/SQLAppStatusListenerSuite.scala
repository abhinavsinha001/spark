...
import com.pubmatic.spark.JobExecutionStatus
...
import com.pubmatic.spark.sql.catalyst.util.quietly
import com.pubmatic.spark.sql.execution.{LeafExecNode, QueryExecution, SQLExecution, SparkPlanInfo}
import com.pubmatic.spark.sql.execution.metric.{SQLMetric, SQLMetrics}
import com.pubmatic.spark.sql.functions.count
import com.pubmatic.spark.sql.internal.StaticSQLConf.UI_RETAINED_EXECUTIONS
import com.pubmatic.spark.sql.test.SharedSparkSession
import com.pubmatic.spark.status.ElementTrackingStore
import com.pubmatic.spark.util.{AccumulatorMetadata, JsonProtocol, LongAccumulator}
import com.pubmatic.spark.util.kvstore.InMemoryStore
...
class SQLAppStatusListenerSuite extends SharedSparkSession with JsonTestUtils
  ...
  test("roundtripping SparkListenerDriverAccumUpdates through JsonProtocol (SPARK-18462)") {
    ...
    assertValidDataInJson(json,
      parse("""
        |{
        |  "Event": "com.pubmatic.spark.sql.execution.ui.SparkListenerDriverAccumUpdates",
        |  "executionId": 1,
        |  "accumUpdates": [[2,3]]
        |}
      """.stripMargin))
...
/**
 * A dummy [[com.pubmatic.spark.sql.execution.SparkPlan]] that updates a [[SQLMetrics]]
 * on the driver.
 */
