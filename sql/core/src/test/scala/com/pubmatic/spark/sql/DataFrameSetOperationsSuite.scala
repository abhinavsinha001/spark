...
import com.pubmatic.spark.sql.catalyst.plans.logical.Union
...
import com.pubmatic.spark.sql.test.SQLTestData.NullStrings
  ...
  test("SPARK-10740: handle nondeterministic expressions correctly for set operations") {
    ...
    def expected(df: DataFrame): Seq[Row] = {
      df.rdd.collectPartitions().zipWithIndex.flatMap {
        case (data, index) =>
          val rng = new com.pubmatic.spark.util.random.XORShiftRandom(7 + index)
