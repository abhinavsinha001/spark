...
import com.pubmatic.spark.sql.{Column, DataFrame, Dataset, Row}
import com.pubmatic.spark.sql.catalyst.expressions.{Literal, Rand, Randn, Shuffle, Uuid}
import com.pubmatic.spark.sql.connector.read.InputPartition
import com.pubmatic.spark.sql.connector.read.streaming.{Offset => OffsetV2}
import com.pubmatic.spark.sql.execution.exchange.ReusedExchangeExec
import com.pubmatic.spark.sql.execution.streaming._
import com.pubmatic.spark.sql.execution.streaming.sources.{MemorySink, TestForeachWriter}
import com.pubmatic.spark.sql.functions._
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.streaming.util.{BlockingSource, MockSourceProvider, StreamManualClock}
  ...
  test("lastProgress should be null when recentProgress is empty") {
    ...
    withTempDir { tempDir =>
      val sq = spark.readStream
        .format("com.pubmatic.spark.sql.streaming.util.BlockingSource")
        .load()
        .writeStream
        .format("com.pubmatic.spark.sql.streaming.util.BlockingSource")
  ...
  test("StreamExecution should call stop() on sources when a stream is stopped") {
    ...
    MockSourceProvider.withMockSources(source) {
      val df = spark.readStream
        .format("com.pubmatic.spark.sql.streaming.util.MockSourceProvider")
        .load()
