...
import com.pubmatic.spark.sql.{AnalysisException, FastOperator}
...
class QueryExecutionSuite extends SharedSparkSession {
  ...
  test("toString() exception/error handling") {
    ...
    assert(qe.toString.contains("com.pubmatic.spark.sql.AnalysisException"))
    ...
    try {
      spark.experimental.extraStrategies = Seq(new SparkStrategy() {
        override def apply(plan: LogicalPlan): Seq[SparkPlan] = {
          plan.foreach {
            case r: com.pubmatic.spark.sql.catalyst.plans.logical.Range =>
