...
import com.pubmatic.spark.sql.execution.ui.{SparkListenerSQLExecutionEnd, SparkListenerSQLExecutionStart}
...
class SQLJsonProtocolSuite extends SparkFunSuite with LocalSparkSession {
  ...
  test("SparkPlanGraph backward compatibility: metadata") {
    val SQLExecutionStartJsonString =
      """
        |{
        |  "Event":"com.pubmatic.spark.sql.execution.ui.SparkListenerSQLExecutionStart",
        |  "executionId":0,
  ...
  test("SparkListenerSQLExecutionEnd backward compatibility") {
    ...
    assert(json == parse(
      """
        |{
        |  "Event" : "com.pubmatic.spark.sql.execution.ui.SparkListenerSQLExecutionEnd",
        |  "executionId" : 1,
        |  "time" : 10
        |}
      """.stripMargin))
