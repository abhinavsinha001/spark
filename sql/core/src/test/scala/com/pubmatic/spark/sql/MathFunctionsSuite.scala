...
import com.pubmatic.spark.sql.functions.{log => logarithm}
...
class MathFunctionsSuite extends QueryTest with SharedSparkSession {
  ...
  test("log / ln") {
    testOneToOneNonNegativeMathFunction(com.pubmatic.spark.sql.functions.log, StrictMath.log)
    checkAnswer(
  ...
  test("binary log") {
    ...
    checkAnswer(
      df.select(com.pubmatic.spark.sql.functions.log("a"),
        com.pubmatic.spark.sql.functions.log(2.0, "a"),
        com.pubmatic.spark.sql.functions.log("b")),
