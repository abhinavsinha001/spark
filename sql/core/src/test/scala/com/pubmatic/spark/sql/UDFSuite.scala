...
import com.pubmatic.spark.sql.api.java._
import com.pubmatic.spark.sql.catalyst.FunctionIdentifier
import com.pubmatic.spark.sql.catalyst.plans.logical.Project
import com.pubmatic.spark.sql.execution.QueryExecution
import com.pubmatic.spark.sql.execution.columnar.InMemoryRelation
import com.pubmatic.spark.sql.execution.command.{CreateDataSourceTableAsSelectCommand, ExplainCommand}
import com.pubmatic.spark.sql.execution.datasources.InsertIntoHadoopFsRelationCommand
import com.pubmatic.spark.sql.functions.{lit, udf}
...
class UDFSuite extends QueryTest with SharedSparkSession {
  ...
  test("SPARK-26308: udf with decimal") {
    ...
    val udf1 = com.pubmatic.spark.sql.functions.udf((value: BigDecimal) => {
  ...
  test("SPARK-26308: udf with complex types of decimal") {
    ...
    val udf1 = com.pubmatic.spark.sql.functions.udf((arr: Seq[BigDecimal]) => {
    ...
    val udf2 = com.pubmatic.spark.sql.functions.udf((map: Map[String, BigDecimal]) => {
  ...
  test("Replace _FUNC_ in UDF ExpressionInfo") {
    ...
    assert(info.getClassName === "com.pubmatic.spark.sql.catalyst.expressions.Upper")
