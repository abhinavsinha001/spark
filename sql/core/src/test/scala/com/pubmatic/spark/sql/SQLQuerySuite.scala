...
package com.pubmatic.spark.sql
...
import com.pubmatic.spark.{AccumulatorSuite, SparkException}
import com.pubmatic.spark.scheduler.{SparkListener, SparkListenerJobStart}
import com.pubmatic.spark.sql.catalyst.optimizer.ConvertToLocalRelation
import com.pubmatic.spark.sql.catalyst.util.StringUtils
import com.pubmatic.spark.sql.execution.HiveResult.hiveResultString
import com.pubmatic.spark.sql.execution.aggregate.{HashAggregateExec, SortAggregateExec}
import com.pubmatic.spark.sql.execution.columnar.InMemoryTableScanExec
import com.pubmatic.spark.sql.execution.command.FunctionsCommand
import com.pubmatic.spark.sql.execution.datasources.v2.BatchScanExec
import com.pubmatic.spark.sql.execution.datasources.v2.orc.OrcScan
import com.pubmatic.spark.sql.execution.datasources.v2.parquet.ParquetScan
import com.pubmatic.spark.sql.execution.joins.{BroadcastHashJoinExec, CartesianProductExec, SortMergeJoinExec}
import com.pubmatic.spark.sql.functions._
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.test.{SharedSparkSession, TestSQLContext}
import com.pubmatic.spark.sql.test.SQLTestData._
...
class SQLQuerySuite extends QueryTest with SharedSparkSession {
  ...
  test("describe functions") {
    checkKeywordsExist(sql("describe function extended upper"),
      "Function: upper",
      "Class: com.pubmatic.spark.sql.catalyst.expressions.Upper",
  ...
  test("using _FUNC_ instead of function names in examples") {
    ...
    val ignoreSet = Set(
      ...
      "com.pubmatic.spark.sql.catalyst.expressions.CaseWhen",
      // _FUNC_ is replaced by `locate` but `locate(... IN ...)` is not supported
      "com.pubmatic.spark.sql.catalyst.expressions.StringLocate",
      // _FUNC_ is replaced by `%` which causes a parsing error on `SELECT %(2, 1.8)`
      "com.pubmatic.spark.sql.catalyst.expressions.Remainder",
      // Examples demonstrate alternative names, see SPARK-20749
      "com.pubmatic.spark.sql.catalyst.expressions.Length")
  ...
  test("check outputs of expression examples") {
    ...
    val ignoreSet = Set(
      ...
      "com.pubmatic.spark.sql.catalyst.expressions.UnixTimestamp",
      // Random output without a seed
      "com.pubmatic.spark.sql.catalyst.expressions.Rand",
      "com.pubmatic.spark.sql.catalyst.expressions.Randn",
      "com.pubmatic.spark.sql.catalyst.expressions.Shuffle",
      "com.pubmatic.spark.sql.catalyst.expressions.Uuid",
      // The example calls methods that return unstable results.
      "com.pubmatic.spark.sql.catalyst.expressions.CallMethodViaReflection")
  ...
  test("SPARK-7158 collect and take return different results") {
    ...
    val idUDF = com.pubmatic.spark.sql.functions.udf(() => UUID.randomUUID().toString)
  ...
  test("run sql directly on files") {
    ...
    withTempPath(f => {
      ...
      checkAnswer(sql(s"select id from json.`${f.getCanonicalPath}`"),
        df)
      checkAnswer(sql(s"select id from `com.pubmatic.spark.sql.json`.`${f.getCanonicalPath}`"),
        df)
    ...
    e = intercept[AnalysisException] {
      sql(s"select id from `com.pubmatic.spark.sql.hive.orc`.`file_path`")
    }
    assert(e.message.contains("Hive built-in ORC data source must be used with Hive support"))

    e = intercept[AnalysisException] {
      sql(s"select id from `com.pubmatic.spark.sql.sources.HadoopFsRelationProvider`.`file_path`")
    }
    assert(e.message.contains("Table or view not found: " +
      "`com.pubmatic.spark.sql.sources.HadoopFsRelationProvider`.file_path"))
    ...
    e = intercept[AnalysisException] {
      sql(s"select id from `com.pubmatic.spark.sql.execution.datasources.jdbc`.`file_path`")
    }
    assert(e.message.contains("Unsupported data source type for direct query on files: " +
      "com.pubmatic.spark.sql.execution.datasources.jdbc"))
  }
