...
package com.pubmatic.spark.sql.streaming
...
import com.pubmatic.spark.SparkEnv
import com.pubmatic.spark.sql.{Dataset, Encoder, QueryTest, Row}
import com.pubmatic.spark.sql.catalyst.encoders.{encoderFor, ExpressionEncoder, RowEncoder}
import com.pubmatic.spark.sql.catalyst.plans.logical.LogicalPlan
import com.pubmatic.spark.sql.catalyst.plans.physical.AllTuples
import com.pubmatic.spark.sql.catalyst.util._
import com.pubmatic.spark.sql.connector.read.streaming.{Offset => OffsetV2, SparkDataStream}
import com.pubmatic.spark.sql.execution.datasources.v2.StreamingDataSourceV2Relation
import com.pubmatic.spark.sql.execution.streaming._
import com.pubmatic.spark.sql.execution.streaming.continuous.{ContinuousExecution, EpochCoordinatorRef, IncrementAndGetEpoch}
import com.pubmatic.spark.sql.execution.streaming.sources.MemorySink
import com.pubmatic.spark.sql.execution.streaming.state.StateStore
import com.pubmatic.spark.sql.streaming.StreamingQueryListener._
import com.pubmatic.spark.sql.test.SharedSparkSession
import com.pubmatic.spark.util.{Clock, SystemClock, Utils}
...
trait StreamTest extends QueryTest with SharedSparkSession with TimeLimits with BeforeAndAfterAll {
  ...
  def testStream(
    ...
    import com.pubmatic.spark.sql.streaming.util.StreamManualClock
