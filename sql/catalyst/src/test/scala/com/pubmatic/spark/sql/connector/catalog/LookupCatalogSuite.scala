 ...
 */
package com.pubmatic.spark.sql.connector.catalog
...
import com.pubmatic.spark.sql.catalyst.analysis.FakeV2SessionCatalog
...
class LookupCatalogSuite extends SparkFunSuite with LookupCatalog with Inside {
  ...
  test("catalog object identifier") {
    Seq(
      ...
      ("`com.pubmatic.spark.sql.json`.`s3://buck/tmp/abc.json`", sessionCatalog,
        Seq("com.pubmatic.spark.sql.json"), "s3://buck/tmp/abc.json")).foreach {
  ...
  test("table identifier") {
    Seq(
      ...
      ("`com.pubmatic.spark.sql.json`.`s3://buck/tmp/abc.json`", "s3://buck/tmp/abc.json",
        Some("com.pubmatic.spark.sql.json"))).foreach {
      case (sql, table, db) =>
        inside (parseMultipartIdentifier(sql)) {
  ...
  test("temporary table identifier") {
    Seq(
      ...
      ("`com.pubmatic.spark.sql.json`.`s3://buck/tmp/abc.json`",
          TableIdentifier("s3://buck/tmp/abc.json", Some("com.pubmatic.spark.sql.json")))).foreach {
...
class LookupCatalogWithDefaultSuite extends SparkFunSuite with LookupCatalog with Inside {
  ...
  test("catalog object identifier") {
    Seq(
      ...
      ("`com.pubmatic.spark.sql.json`.`s3://buck/tmp/abc.json`", catalogs("prod"),
          Seq("com.pubmatic.spark.sql.json"), "s3://buck/tmp/abc.json")).foreach {
      case (sql, expectedCatalog, namespace, name) =>
        inside(parseMultipartIdentifier(sql)) {
          case CatalogObjectIdentifier(catalog, ident) =>
