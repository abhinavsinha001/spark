  ...
  test("the real type is not compatible with encoder schema: primitive array") {
    ...
    assert(intercept[AnalysisException](encoder.resolveAndBind(attrs)).message ==
      s"""
         ...
         |- root class: "com.pubmatic.spark.sql.catalyst.encoders.PrimitiveArrayClass"
  ...
  test("throw exception if real type is not compatible with encoder schema") {
    ...
    assert(msg1 ==
      s"""
         ...
         |- root class: "com.pubmatic.spark.sql.catalyst.encoders.StringIntClass"
         |You can either add an explicit cast to the input data or choose a higher precision type
       """.stripMargin.trim + " of the field in the target object")
    ...
    assert(msg2 ==
      s"""
         ...
         |- field (class: "com.pubmatic.spark.sql.catalyst.encoders.StringLongClass", name: "b")
         |- root class: "com.pubmatic.spark.sql.catalyst.encoders.ComplexClass"
         |You can either add an explicit cast to the input data or choose a higher precision type
       """.stripMargin.trim + " of the field in the target object")
  }
