...
import com.pubmatic.spark.sql.catalyst.analysis.UnresolvedExtractValue
import com.pubmatic.spark.sql.catalyst.expressions.{CreateNamedStruct, Expression, If, SpecificInternalRow, UpCast}
import com.pubmatic.spark.sql.catalyst.expressions.objects.{AssertNotNull, NewInstance}
...
class ScalaReflectionSuite extends SparkFunSuite {
  ...
  test("SPARK-15062: Get correct serializer for List[_]") {
    ...
    assert(serializer.asInstanceOf[NewInstance]
      .cls.isAssignableFrom(classOf[com.pubmatic.spark.sql.catalyst.util.GenericArrayData]))
  }
