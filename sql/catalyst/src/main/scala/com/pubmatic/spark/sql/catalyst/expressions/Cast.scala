...
import com.pubmatic.spark.sql.catalyst.analysis.{TypeCheckResult, TypeCoercion}
...
import com.pubmatic.spark.sql.catalyst.util.DateTimeConstants._
import com.pubmatic.spark.sql.catalyst.util.DateTimeUtils._
import com.pubmatic.spark.sql.catalyst.util.IntervalUtils._
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.internal.SQLConf.IntervalStyle._
...
abstract class CastBase extends UnaryExpression with TimeZoneAwareExpression with NullIntolerant {
    ...
    from match {
      ...
      case TimestampType =>
        ...
        (c, evPrim, evNull) => code"""$evPrim = UTF8String.fromString(
          com.pubmatic.spark.sql.catalyst.util.DateTimeUtils.timestampToString($tf, $c));"""
  ...
  private[this] def castToDateCode(
    ...
    from match {
      case StringType =>
        ...
        (c, evPrim, evNull) =>
          ...
          scala.Option<Integer> $intOpt =
            com.pubmatic.spark.sql.catalyst.util.DateTimeUtils.stringToDate($c, $zid);
          if ($intOpt.isDefined()) {
      ...
      case TimestampType =>
        ...
        (c, evPrim, evNull) =>
          code"""$evPrim =
            com.pubmatic.spark.sql.catalyst.util.DateTimeUtils.microsToEpochDays($c, $zid);"""
      case _ =>
  ...
  private[this] def castToTimestampCode(
      ...
      (c, evPrim, evNull) =>
        code"""
          scala.Option<Long> $longOpt =
            com.pubmatic.spark.sql.catalyst.util.DateTimeUtils.stringToTimestamp($c, $zid);
    ...
    case DateType =>
      ...
      (c, evPrim, evNull) =>
        code"""$evPrim =
          com.pubmatic.spark.sql.catalyst.util.DateTimeUtils.epochDaysToMicros($c, $zid);"""
    case DecimalType() =>
