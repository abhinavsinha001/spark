...
import com.pubmatic.spark.{SPARK_REVISION, SPARK_VERSION_SHORT}
...
import com.pubmatic.spark.sql.catalyst.util.RandomUUIDGenerator
...
case class Uuid(randomSeed: Option[Long] = None) extends LeafExpression with Stateful
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    ctx.addMutableState("com.pubmatic.spark.sql.catalyst.util.RandomUUIDGenerator", randomGen,
      forceInline = true,
      useFreshName = false)
    ctx.addPartitionInitializationStatement(s"$randomGen = " +
      "new com.pubmatic.spark.sql.catalyst.util.RandomUUIDGenerator(" +
