...
import com.pubmatic.spark.sql.catalyst.util.{DateTimeUtils, TimestampFormatter}
...
case class MakeTimestamp(
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    nullSafeCodeGen(ctx, ev, (year, month, day, hour, min, secAndNanos, timezone) => {
      ...
      s"""
      try {
        com.pubmatic.spark.sql.types.Decimal secFloor = $secAndNanos.floor();
        com.pubmatic.spark.sql.types.Decimal nanosPerSec = $d$$.MODULE$$.apply(1000000000L, 10, 0);
