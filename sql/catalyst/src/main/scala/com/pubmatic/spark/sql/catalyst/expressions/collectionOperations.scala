...
import com.pubmatic.spark.sql.catalyst.expressions.ArraySortLike.NullOrder
...
import com.pubmatic.spark.util.collection.OpenHashSet
  ...
  private class TemporalSequenceImpl[T: ClassTag]
    ...
    override def genCode(
      ...
      s"""
         ...
         |    $i += 1;
         |    $t = com.pubmatic.spark.sql.catalyst.util.DateTimeUtils.timestampAddInterval(
