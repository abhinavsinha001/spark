...
package com.pubmatic.spark.sql.catalyst.catalog
...
import com.pubmatic.spark.sql.catalyst.analysis.FunctionRegistry.FunctionBuilder
import com.pubmatic.spark.sql.catalyst.expressions.{Expression, ExpressionInfo, ImplicitCastInputTypes}
import com.pubmatic.spark.sql.catalyst.parser.{CatalystSqlParser, ParserInterface}
import com.pubmatic.spark.sql.catalyst.plans.logical.{LogicalPlan, SubqueryAlias, View}
...
class SessionCatalog(
  ...
  protected def makeFunctionExpression(
    ...
    val clsForUDAF =
      Utils.classForName("com.pubmatic.spark.sql.expressions.UserDefinedAggregateFunction")
    if (clsForUDAF.isAssignableFrom(clazz)) {
      val cls = Utils.classForName("com.pubmatic.spark.sql.execution.aggregate.ScalaUDAF")
