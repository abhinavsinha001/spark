...
import com.pubmatic.spark.sql.catalyst.util.{ArrayData, GenericArrayData, TypeUtils}
...
case class Overlay(input: Expression, replace: Expression, pos: Expression, len: Expression)
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    defineCodeGen(ctx, ev, (input, replace, pos, len) =>
      "com.pubmatic.spark.sql.catalyst.expressions.Overlay" +
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    ...
    nullSafeCodeGen(ctx, ev, (src, matching, replace) => {
      ...
      s"""if ($check) {
        ...
        $termDict = com.pubmatic.spark.sql.catalyst.expressions.StringTranslate
