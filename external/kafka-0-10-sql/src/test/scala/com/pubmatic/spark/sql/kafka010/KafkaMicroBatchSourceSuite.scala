...
package com.pubmatic.spark.sql.kafka010
...
import com.pubmatic.spark.sql.{Dataset, ForeachWriter, Row, SparkSession}
import com.pubmatic.spark.sql.catalyst.util.CaseInsensitiveMap
import com.pubmatic.spark.sql.connector.read.streaming.SparkDataStream
import com.pubmatic.spark.sql.execution.datasources.v2.StreamingDataSourceV2Relation
import com.pubmatic.spark.sql.execution.exchange.ReusedExchangeExec
import com.pubmatic.spark.sql.execution.streaming._
import com.pubmatic.spark.sql.execution.streaming.continuous.ContinuousExecution
import com.pubmatic.spark.sql.functions.{count, window}
import com.pubmatic.spark.sql.internal.SQLConf
import com.pubmatic.spark.sql.kafka010.KafkaSourceProvider._
import com.pubmatic.spark.sql.streaming.{StreamTest, Trigger}
...
abstract class KafkaMicroBatchSourceSuiteBase extends KafkaSourceSuiteBase {
  ...
  test("SPARK-26718 Rate limit set to Long.Max should not overflow integer " +
    ...
    val kafka = spark
      ...
      .as[(String, String)]
    val mapped: com.pubmatic.spark.sql.Dataset[_] = kafka.map(kv => kv._2.toInt)
  ...
  test("SPARK-22956: currentPartitionOffsets should be set when no new data comes in") {
    def getSpecificDF(range: Range.Inclusive): com.pubmatic.spark.sql.Dataset[Int] = {
      val topic = newTopic()
      testUtils.createTopic(topic, partitions = 1)
