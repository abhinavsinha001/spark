...
import com.pubmatic.spark.sql.catalyst.expressions.{AttributeReference, SpecificInternalRow, UnsafeProjection}
import com.pubmatic.spark.sql.execution.streaming.{MemoryStream, MemoryStreamBase}
...
import com.pubmatic.spark.sql.test.{SharedSparkSession, TestSparkSession}
import com.pubmatic.spark.sql.types.{BinaryType, DataType, IntegerType, StringType, StructField, StructType}
...
abstract class KafkaSinkBatchSuiteBase extends KafkaSinkSuiteBase {
  ...
  test("batch - partition column and partitioner priorities") {
    ...
    val customKafkaPartitionerConf = Map(
      "kafka.partitioner.class" -> "com.pubmatic.spark.sql.kafka010.TestKafkaPartitioner"
    )
