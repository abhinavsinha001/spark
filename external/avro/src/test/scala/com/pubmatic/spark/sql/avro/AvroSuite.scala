...
package com.pubmatic.spark.sql.avro
...
import com.pubmatic.spark.sql.TestingUDT.{IntervalData, NullData, NullUDT}
...
abstract class AvroSuite extends QueryTest with SharedSparkSession {
  ...
  test("resolve avro data source") {
    ...
    Seq("com.pubmatic.spark.sql.avro.AvroFileFormat", databricksAvro).foreach { provider =>
      assert(DataSource.lookupDataSource(provider, spark.sessionState.conf) ===
        classOf[com.pubmatic.spark.sql.avro.AvroFileFormat])
    }
  ...
  test("support user provided avro schema for writing nullable enum type") {
    withTempPath { tempDir =>
      ...
      assert(message.contains("com.pubmatic.spark.sql.avro.IncompatibleSchemaException: " +
  ...
  test("support user provided avro schema for writing non-nullable enum type") {
    withTempPath { tempDir =>
      ...
      assert(message2.contains("com.pubmatic.spark.sql.avro.IncompatibleSchemaException: " +
        "Cannot write \"NOT-IN-ENUM\" since it's not defined in enum"))
    }
  }
  ...
  test("support user provided avro schema for writing nullable fixed type") {
    withTempPath { tempDir =>
      ...
      assert(message1.contains("com.pubmatic.spark.sql.avro.IncompatibleSchemaException: " +
  ...
  test("throw exception if unable to write with user provided Avro schema") {
    ...
    def assertException(f: () => AvroSerializer): Unit = {
      val message = intercept[com.pubmatic.spark.sql.avro.IncompatibleSchemaException] {
        f()
      }.getMessage
