...
import com.pubmatic.spark.streaming.dstream.{DStream, InputDStream}
...
class StreamingContextSuite
  ...
  test("getActiveOrCreate with checkpoint") {
    ...
    testGetActiveOrCreate {
      ssc = new StreamingContext(
        conf.clone.set("spark.streaming.clock", "com.pubmatic.spark.util.ManualClock"), batchDuration)
      addInputStream(ssc).register()
      ssc.start()
  ...
  test("multiple streaming contexts") {
    val sc = new SparkContext(
      conf.clone.set("spark.streaming.clock", "com.pubmatic.spark.util.ManualClock"))
