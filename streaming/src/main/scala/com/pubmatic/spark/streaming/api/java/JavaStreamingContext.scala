...
import com.pubmatic.spark.api.java.function.{Function, Function0, Function2}
...
import com.pubmatic.spark.api.java.{JavaPairRDD, JavaRDD, JavaSparkContext}
import com.pubmatic.spark.api.java.function.{Function => JFunction}
import com.pubmatic.spark.api.java.function.{Function0 => JFunction0}
...
import com.pubmatic.spark.streaming.scheduler.StreamingListener
...
class JavaStreamingContext(val ssc: StreamingContext) extends Closeable {
  ...
  def socketStream[T](
                       hostname: String,
                       port: Int,
                       converter: Function[InputStream, java.lang.Iterable[T]],
                       storageLevel: StorageLevel)
  ...
  def transform[T](
      ...
      transformFunc: Function2[JList[JavaRDD[_]], Time, JavaRDD[T]]
    ): JavaDStream[T] = {
    implicit val cmt: ClassTag[T] =
      implicitly[ClassTag[AnyRef]].asInstanceOf[ClassTag[T]]
  ...
  def transformToPair[K, V](
      dstreams: JList[JavaDStream[_]],
      transformFunc: Function2[JList[JavaRDD[_]], Time, JavaPairRDD[K, V]]
    ): JavaPairDStream[K, V] = {
...
object JavaStreamingContext {
  ...
  def getOrCreate(
      ...
      creatingFunc: Function0[JavaStreamingContext]
